...
impl<P: LazyMapMonoid> Node<P> {
    ...
    fn recalc(&mut self) {
        ...
        let (left_size, left_agg, left_rev) = match self.left.as_ref() {
            Some(left) => (left.size, &left.agg, &left.agg_rev),
            None => (0, &unit, &unit),
        };
        let (right_size, right_agg, right_rev) = match self.right.as_ref() {
            Some(right) => (right.size, &right.agg, &right.agg_rev),
            None => (0, &unit, &unit),
        };
    ...
    fn recalc_black_height(&mut self) {
        let left_bh = self.left.as_ref().map_or(0, |n| n.black_height);
        let right_bh = self.right.as_ref().map_or(0, |n| n.black_height);
        self.black_height = left_bh.max(right_bh) + u8::from(!self.red);
    }
    ...
    fn push(&mut self) {
        ...
        if self.lazy_pending {
            // Move out `lazy` to avoid cloning potentially large actions.
            let act = std::mem::replace(&mut self.lazy, P::act_unit());
            if let Some(left) = self.left.as_deref_mut() {
